                    ...
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, { Error: 'Errors encountered while attempting to delete all of the use\'s checks. All checks may not have been deleted from the system successfully.' });
                        }
                      }
                    })
                  })
...
// Checks - put
// Required data: id
// Optional data: proltocol, url, method, sucessCodes, timeoutSeconds (one must be sent)
            ...
            if (!err) {
              ...
              _data.read('users', checkData.userPhone, (err, userData) => {
                if (!err && userData) {
                  ...
                  callback(500, { Error: 'Could not find the user who created the check, so could not remove the check from the list of checks on the use object' });
                }
              });
            } else {
